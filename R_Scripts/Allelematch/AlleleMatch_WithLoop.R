...
data = as.data.frame(read.table("testfile3.txt", header = T, sep = "\t")) #Reads testfile2.txt as the focal dataset
missingValue = -99

for(i in seq(1,nrow(data),2)) #Starts the loop for every other row (since we have 2 deer replicate pcrs
{
genotype=as.data.frame(data[i,1]) #Initialize the data frame using the name of that sample
for(j in seq(2,ncol(data),2)) #Initialize the loop for every 2 columns (each locus in 2 column format)
{
	if(length(sort(unique(c(data[i,j],data[i,j+1],data[i+1,j],data[i+1,j+1]))))>=2) {
		obj1=sort(unique(c(data[i,j],data[i,j+1],data[i+1,j],data[i+1,j+1])))
...
genotype=as.data.frame(c(genotype,obj1))
}
print(genotype)
}
...
test = lapply(names, test1)


##############ABOVE THIS WORKS, DON'T CHANGE IT#########################


#############BELOW THIS IS THE FINAL SCRIPT###############################
