...
DNA = read.phyDat("test_YChrom2.fas", format = "fasta", type = "DNA") #turns YourFile.fas from your wd into an object 'DNA', class phyDat
...
modelTest(fit)
fit1 = optim.pml(fit, optNni = TRUE, model = "K80", optInv=T) #optimizes the tree and makes it an object 'fit1'. You can specify a nucleotide substitution
...
bs = bootstrap.pml(fit1, bs = 500, optNni = TRUE) #performs 50 bootstrap replicates on your new optimized tree, 'fit1'
...
plotBS(root, bs, bs.adj = c(1.1,1), type="phylogram", cex=1, bs.cex=1, bs.col="gray50", font = 1, use.edge.length = T, edge.color="black", tip.color="black", label.offset = 0.001, root.edge=T)
