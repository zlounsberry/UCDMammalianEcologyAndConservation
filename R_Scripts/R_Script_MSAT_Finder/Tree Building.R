...
AlbTree = read.phyDat("Alb Seq.fas", format = "fasta", type = "DNA")
...
layout(matrix(c(1,2), 2, 1), height=c(1,2)) #Creates the layout matrix to print tree to
par(mar = c(.1,.1,.1,.1)) #Parameters of margins that out
plot(treeUPGMA, main="UPGMA") #Plots your rooted UPGMA tree
plot(treeNJ, "unrooted", main="NJ") #Plots unrooted NJ tree
...
DNA = read.phyDat("AlbUnrooted.fas", format = "fasta", type = "DNA")
DNA
dm = dist.ml(DNA)
tree = fastme.bal(dm)
fit = pml(tree, DNA)
