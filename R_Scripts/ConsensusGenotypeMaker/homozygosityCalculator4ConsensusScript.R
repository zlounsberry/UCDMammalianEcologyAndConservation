...
data_full <- read.table("TehamaDeer_5LociFailed.txt", header = T, sep = "\t")
...
write.table(all, "TehamaDeer_5LociFailed_Output.txt", sep="\t", row.names=F)


##Now remove samples with too many homozygous samples (likely crummy samples)
all
