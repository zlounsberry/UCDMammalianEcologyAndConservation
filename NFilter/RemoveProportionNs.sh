...
y=($(wc -l FILE.fasta)) # Makes a variable 'y' that is the number of lines in your infile
...
sequence=($(awk "NR==${c1}" FILE.fasta)) #Store every other sequence iteratively as a variable
...
if (( $(echo "$Prop > 0.5" | bc -l) )) # If your proportion is greater than your cutoff (here 50%), print to a file TooManyNs.fasta. Otherwise print to Saved.fasta (the file you want to use)
        then
